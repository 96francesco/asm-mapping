...
class Sentinel1Dataset(Dataset): #type: ignore
      """
      ...
      def _setup_folders(self) -> Tuple[str, Optional[str]]:
            ...
            gt_folder = os.path.join(self.data_dir, config['gt_subfolder']) if config['use_gt'] else None
      ...
      @staticmethod
      def vv_vh_ratio(img: np.ndarray[Any, Any]) -> Any:
            vv = img[0, :, :]
            vh = img[1, :, :] 
            ratio = vv - vh  # subtraction because of the dB scale
